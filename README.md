...
**First and foremost:** this project requires installation of both [Python](https://www.python.org/downloads/) (3.10.6+) and [Django](https://www.djangoproject.com/download/) (4.1.3+).
**Database engine:** It also requires access to a database through a user with enough privileges to create new tables and manipulate the data therein ([from the docs](https://docs.djangoproject.com/en/4.1/ref/databases/)). The project is currently set to work with MySQL, but switching to another supported backend should be an easy fix.
**Data migration:** it is necessary to tell Django to create the tables in the database form the models defined in the project. To do this, open a terminal window at the project's root directory and type:
  $ python manage.py migrate
  $ python manage.py makemigrations
Once the tables are created, it is possible to populate them with pre-dumped data to start playing with the app right-away:
  $ python manage.py loaddata data.json
data.json is a fixture, a JSON object that tells Django what data to use to populate the tables in the database.
**Discogs' API:** this app fetches its music data from the Discogs database through their official API. It is free to use, but it does require user authentication in a form of a token. In this project, the file polls/views.py, on line 96, reads the user_token string from a local file that is gitignored. In order to create more polls than the ones provided with the app by default, it is necessary to create a Discogs user account and request a token.
...
- **View:** views are Python functions, or classes, written to respond to web requests. They map to specific URL endpoints and are set to handle requests according to the implemented logic that lives within it.


- **Template:** the framework offers the Django Template Language, DTL, which makes it possible to write text files containing placeholders that can be dynamically rendered as part of a web response.


## Part 3: Report

This project was developed based on the web framework Django and its MVT design pattern. As of now, it only offers one application, the Polls app, which allows for the creation of music related polls that can then be voted by other users. The following scheme explains the organization of its main components:
...
- **1. Models**  
  ...
  - **1.2. Choice**  
  This class represents the Choice entity in the database. Each choice is a music album whose genre (at least one, because an album can have many) and year match the ones defined in the poll's question the choice belongs to. Its attributes are:
    - question: the database ID of the question that the choice belongs to.
    - image: the URL to the album cover image.
    - title: the album's title.
    - artist: the album's main artist.
    - year: the release year of the album (matches its parent question's).
    - genres: the list of genres of the album (at least one matches its parent question's).
  ...
  Other models used in this project, such as the User model, are provided out-of-the-box by Django itself. Access [here](https://docs.djangoproject.com/en/4.1/ref/contrib/auth/) for more information.

- **2. Views**  
  - **2.1 accounts/SignUpView**  
  ...
  View restrictions: this view has no restrictions.
  - **2.2 accounts/DetailView**  
  ...
  View restrictions: user must be logged in; test_func must be true.
  - **2.3 accounts/DeleteView**  
  ...
  View restrictions: user must be logged in.
  - **2.9 polls/vote**  
  This function controls user requests to the associated endpoint defined in the URL pattern .../polls/<poll_id>/vote/. It handles POST requests with the ID of the chosen album by incrementing by one its votes column in the database.
  View restrictions: user must be logged in.


- **3. Templates**  
  - **3.1 accounts/change_email.html**  
  ...
  - **3.3 accounts/delete_confirmation.html**  
  ...
  - **3.7 polls/index.html**  
  This HTML file gets dynamically inflated by the polls/IndexView view and displays a list of the 5 most recently created polls, as well as input box that filters polls by genre and year.
  - **3.8 polls/no_matches.html**  
  ...
  - **3.13 registration/password_change_form.html**  
  ...
  - **3.17 registration/password_reset_form.html**  
